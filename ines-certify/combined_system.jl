#Note: rolling horizon and stochastics are disabled
...
ps__factor_wind = Dict(
    :cheap_low => ps__wind,
    :cheap_high => ps__wind,#(ps__wind .+ 0.25)/1.25,
    :expensive_low => ps__wind,
    :expensive_high => ps__wind,#(ps__wind .+ 0.25)/1.25
)
ps__solar = [round((sin(pi/2*t)+1)/2;digits=2) for t in s__t]
ps__factor_solar = Dict(
    :cheap_low => ps__solar,
    :cheap_high => ps__solar,#(ps__solar .+ 0.25)/1.25,
    :expensive_low => ps__solar,
    :expensive_high => ps__solar,#(ps__solar .+ 0.25)/1.25
)
...
p__flow_profile__gas_demand = Dict(s=> Dict(t => 20.0*ps__factor_demand[s] for t in s__t_gas) for s in s__s)
p__flow_profile__electricity_demand = Dict(s=> Dict(t => 20.0*ps__factor_demand[s] for t in s__t) for s in s__s)
...
p__max_units__solar_unit = 1.0
p__capacity__solar_unit = 10.0

#p__investment_cost__battery = 800.0
p__capacity__battery = 50.0
p__existing_units__battery = 1.0
...
@variable(model,p__existing_units__gas_unit<=v__investment__gas_unit<=p__max_units__gas_unit)
@variable(model,0<=v__flow__gas_unit_in[s__s,s__t_gas])
@variable(model,0<=v__flow__gas_unit_out[s__s,s__t])

@variable(model,p__existing_units__wind_unit<=v__investment__wind_unit<=p__max_units__wind_unit)
@variable(model,0<=v__flow__wind_unit[s__s,s__t])
@variable(model,p__existing_units__solar_unit<=v__investment__solar_unit<=p__max_units__solar_unit)
...
@constraint(model,c__balance__gas_demand[s in s__s, t in s__t_gas],p__flow_profile__gas_demand[s][t] == v__flow__gas_distribution_out[s,t])
...
@constraint(model,c__balance__electricity_demand[s in s__s, t in s__t],p__flow_profile__electricity_demand[s][t] == v__flow__electricity_distribution_out[s,t]+v__flow__solar_unit[s,t])
@constraint(model,c__balance__electricity_hub[s in s__s, t in s__t],v__flow__electricity_distribution_in[s,t]+v__charge__battery[s,t] == v__flow__electricity_transport_out[s,t] + v__flow__gas_unit_out[s,t] + v__flow__nuclear_unit[s,t]+p__efficiency_discharge__battery*v__discharge__battery[s,t])
...
#@constraint(model,c__on_init__nuclear_unit[s in s__s],v__on__nuclear_unit[s,s__t[1]]==v__on_up__nuclear_unit[s,s__t[1]]-v__on_down__nuclear_unit[s,s__t[1]])
...
@constraint(model,c__capacity_charge__battery[s in s__s, t in s__t],v__charge__battery[s,t]<=p__capacity_charge__battery*p__existing_units__battery)
@constraint(model,c__capacity_discharge__battery[s in s__s, t in s__t],v__discharge__battery[s,t]<=p__capacity_discharge__battery*p__existing_units__battery)
@constraint(model,c__capacity__battery[s in s__s, t in s__t],v__state__battery[s,t]<=p__capacity__battery*p__existing_units__battery)
...
println("FLOWS from gas, solar, wind")
println(JuMP.value(v__flow__gas_unit_out))
println(JuMP.value(v__flow__solar_unit))
println(JuMP.value(v__flow__wind_unit))

println("IMPORT of electricity and gas")
println(JuMP.value(v__flow__electricity_import))
println(JuMP.value(v__flow__gas_import))

println("NUCLEAR")
println(JuMP.value(v__flow__nuclear_unit))
println(JuMP.value(v__on__nuclear_unit))
println(JuMP.value(v__on_up__nuclear_unit))
